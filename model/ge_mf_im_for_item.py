...
class GEMF(MF):
    ...
    def __init__(self):
        ...
        weight = 0.5
        file = '%s_CInet' % self.config.dataset_name
        ...
        self.config.walk_result_path = '../data/ge/' + file + '_social_corpus_im_only_item.txt'

        # parameters for graph embedding
        self.config.lambdaW = 1
        self.config.table_path = '../data/ge/' + file + '_table_im_only_item.pkl'
        self.config.model_out_path = '../data/ge/' + file + '_result_im_only_item.txt'
        self.config.cbow = 0
        self.config.neg = 5
    ...
    def reset_index(self): # 141个social用户没有在ui中出现,需要将他们初始化然后并到P中去
        print('the current item number in ui is ' + str(len(self.rg.item)))
        not_exists_ui = [] # 记录不在ui中的item list
        num = 0
        mapping = {'<bol>': -1, '<eol>': -2, '<unk>': -3}
        for item in self.social_vocab.vocab_hash:
            if item != '<bol>' and item != '<eol>' and item != '<unk>' and not int(item) in self.rg.item:
                num += 1
                not_exists_ui.append(int(item))
                # 若社交语料的item不在ui阵里面，则将这个item扩充到训练集中，更新其编号，并且赋值给社交item字典
                self.rg.item[int(item)] = len(self.rg.item) # 扩充self.rg.item
                self.social_vocab.vocab_hash[item] = self.rg.item[int(item)] # reset the index of items in social corpus in order to be common in ui
            elif item != '<bol>' and item != '<eol>' and item != '<unk>':
                # 若社交语料的item在ui阵里面，则将这个item在ui阵训练集中的编号赋值给社交item字典
                self.social_vocab.vocab_hash[item] = self.rg.item[int(item)] # reset the index of items in social corpus
            else:
                # 处理三个特殊字符的编号
                index = mapping[item]
                self.rg.item[index] = len(self.rg.item)
                self.social_vocab.vocab_hash[item] = self.rg.item[index]
        print("the number of not exists in ui is "+ str(num))
    ...
    def init_net(self):
        ...
        self.Q = np.random.rand(self.rg.get_train_size()[1], self.config.factor) / (  # 跟随机初始化的变量有很大关系
                self.config.factor ** 0.5)  # the common user latent vetors in MF and GE
        self.W = np.random.rand(self.rg.get_train_size()[1], self.config.factor) / (
        self.config.factor ** 0.5)   # the common user latent vetors in MF and GE
    ...
    def train_model(self):
        ...
        while iteration < self.config.maxIter:
            ...
            for index, line in enumerate(self.rg.trainSet()):
                ...
                walks_list = self.node_inverted_index[str(item)]
                wl = len(walks_list)
                ...
                # for walk_line in walks_list:
                    ...
                    for sent_pos, token in enumerate(sent):
                        ...
                        self.pos_loss_total += np.log(sigmoid(np.dot(self.Q[token], self.pos_neu1)))
                        self.neg_loss_total += np.log(sigmoid(- np.dot(self.Q[token], self.neg_neul)))
                        ...
                        for target, label in classifiers:
                            if label == 1:
                                ...
                                z_po = np.dot(self.pos_neu1, self.Q[token])
                                p_po = sigmoid(z_po)
                                f_po = self.config.alpha * (label - p_po)
                                g_po = f_po * self.Q[token] - self.config.lambdaW * self.W[target]  # 负梯度
                                self.W[target] += self.config.w2v_lr * g_po
                            else:
                                z_ne = np.dot(self.neg_neul, self.Q[token])
                                p_ne = sigmoid(z_ne)
                                f_ne = self.config.alpha * (label - p_ne)
                                g_ne = f_ne * self.Q[token] - self.config.lambdaW * self.W[target]  # 负梯度
                ...
                self.Q[i] += self.config.lr * (
                        error * p + self.config.alpha * (self.pos_neu1 * (1 - sigmoid(np.dot(self.pos_neu1, self.Q[i])))
                                                         - self.neg_neul * sigmoid(
                            np.dot(self.neg_neul, self.Q[i]))) - self.config.lambdaQ * q)
